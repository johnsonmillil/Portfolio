...
router.get("/attendees/:id", authMiddleWare, (req, res, next) => {
  events.findById(req.params.id, (error, event) => {
    if (error) {
      ...
      if (!event) res.status(400).send("Event not found");
      else {
...
// GET event data in an async fashion.
async function getEventData(id) {
  const evnt = await events.findOne({ _id: id, org: org });

  return evnt;
}

// GET clients data in an async fashion.
// Had to wrap it in Promise because the call back.
async function getClientData(id) {
  try {
    const clnts = await new Promise((resolve, reject) => {
      events.findById(id, (error, event) => {
        if (error) {
          reject(error);
        } else {
          if (!event) reject(error);
          else {
            const attendeeIds = event.attendees.map((attendee) =>
              attendee.toString()
            );
...
// GET services data in an async fashion.
async function getServices() {
  const srv = await services.find({ org: org });
  return srv;
}

// GET all details of an event by event ID.
router.get("/event/:id", authMiddleWare, async (req, res, next) => {
  resultData = {};
  resultData.event = await getEventData(req.params.id);
  resultData.clients = await getClientData(req.params.id);
  resultData.services = await getServices();
